...
let findi x env = (match Map.find x env with Int(i) -> i | _ -> failwith "Not found")
let findf x env = (match Map.find x env with Float(d) -> d | _ -> failwith "Not found")
let findt x env = (match Map.find x env with Tuple(ys) -> ys | _ -> failwith "Not found")
